...
public class BillDaoImpl implements BillDao{
	...
	public void insert(Bill bill) {
		...
		try {
			...
			ResultSet generatedKeys = preparedStatement.getGeneratedKeys();//lay id dc tao tu dong trong DB ra de luu vao billproduct
	...
	public Bill get(int id) {
		...
		try {
			String sql="SELECT b.id as 'id', u.name as 'name', u.age as 'age',u.address as 'address' ,u.id as 'iduser',b.buydate as 'buydate', b.pricetotal as 'pricetotal'"
					+ " FROM javacore14.bill b " + 
					"inner join user u " + 
					"on b.id_user=u.id where u.id=?";
			PreparedStatement preparedStatement= connection.prepareStatement(sql);
			preparedStatement.setInt(1, id);
		...
		}catch(Exception e) {System.out.println("get loi"+e.getMessage());}
		return null;
	}
	...
	public List<Bill> search(String findName) {
		...
		try {
			String sql="SELECT b.id as 'id', u.name as 'name', u.age as 'age',u.address as 'address' ,b.buydate as 'buydate', b.pricetotal as 'pricetotal'"
					+ " FROM javacore14.bill b " + 
					"inner join user u " + 
					"on b.id_user=u.id where u.name like?";
			PreparedStatement preparedStatement= connection.prepareStatement(sql);
		...
		}catch(Exception e) {System.out.println("search loi"+e.getMessage());}
		return list;
	}
