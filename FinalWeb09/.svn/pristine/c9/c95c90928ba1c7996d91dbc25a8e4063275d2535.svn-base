...
public class BillProductDaoImpl implements BillProductDao {
	...
	public BillProduct get(int id) {
		...
		try {
			String sql = "SELECT bp.id as 'id', b.id as 'idbill' ,b.id_user as 'iduser' "
					...
					+ "FROM javacore14.billproduct bp " + "inner join bill b " + "on b.id= bp.id_bill "
					+ "inner join product p " + "on bp.id_product= p.id where bp.id=?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setInt(1, id);
			...
			System.out.println("get loi>>" + exception.getMessage());
		}
		return null;
	}
	...
	public List<BillProduct> search(String findName) {
		...
		try {
			String sql = "SELECT bp.id as 'id', b.id as 'idbill' ,b.id_user as 'iduser' "
					...
					+ "FROM javacore14.billproduct bp " + "inner join bill b " + "on b.id= bp.id_bill "
					+ "inner join product p " + "on bp.id_product= p.id where p.name like ?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
	...
	@Override
	public List<BillProduct> searchtheoidbill(int idBill) {
		List<BillProduct> list = new ArrayList<>();
		Connection connection = JDBCConnection.getConn();
		try {
			String sql = "SELECT bp.id as 'id', b.id as 'idbill' ,b.id_user as 'iduser' "
					...
					+ ",bp.quantity as 'quantitybuy', bp.unitprice as 'priceonebillproduct' "
					+ "FROM javacore14.billproduct bp " + "inner join bill b " + "on b.id= bp.id_bill "
					+ "inner join product p " + "on bp.id_product= p.id where b.id =?";
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
