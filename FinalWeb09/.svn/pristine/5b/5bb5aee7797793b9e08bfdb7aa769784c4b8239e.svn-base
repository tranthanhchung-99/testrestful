<%@page import="com.ttjv.model.BillProduct"%>
...
<body>
	<%
		BillProduct billProduct = (BillProduct) request.getAttribute("billProduct");
	%>
	<form action="/FinalWeb09/billproduct/update" method="post">
		...
		<div>
			<lable>ID_BILL</lable>
			<input type="text" name="idBill"
				value="<%=billProduct.getBill().getId()%>">
		</div>
		<div>
			<lable>ID_PRODUCT</lable>
			<input type="text" name="idProduct"
				vabillProduct.getProduct().getId()getId() %>">
		</div>
		<div>
